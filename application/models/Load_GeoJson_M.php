...
//Loop through rows to build feature arrays
 ...
 function kehoach_thamkhao_chitiet_duongdi(){
      $query = $this->db->query(" SELECT c.gid,c.x,c.y,c.ten_kh,c.mota,c.stt  from ((SELECT kh.ten_kh,ct.ma_kh,ct.gid,ct.mota,ct.stt,d.x, d.y from kehoach_chitiet as ct, kehoach_dulich as kh, diem_dulich_hcm as d where ct.gid=d.gid and kh.ma_kh=ct.ma_kh order by ct.stt desc) union (SELECT kh.ten_kh,ct.ma_kh,ct.gid,ct.mota,ct.stt, d.x, d.y from kehoach_chitiet_duongdi as ct, kehoach_dulich as kh, diem_duongdi as d where ct.gid=d.gid and kh.ma_kh=ct.ma_kh order by ct.stt desc)) as c order by stt asc");
      $geojson = array(
        'type'      => 'FeatureCollection',
        'features'  => array()
        );
